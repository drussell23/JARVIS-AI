...
class IntelligentJARVISVision:
    ...
    def __init__(self):
        ...
        if self.api_key:
            try:
                from .optimized_claude_vision import OptimizedClaudeVisionAnalyzer

                self.claude_analyzer = OptimizedClaudeVisionAnalyzer(self.api_key)
            except ImportError:
    ...
    async def handle_intelligent_command(self, command: str) -> str:
        ...
        if (
            ...
            or "what's on my screen" in command_lower
        ):
            ...
            if self.api_key:
                try:
                    ...
                    if claude_result.get("intelligence_used") and claude_result.get(
                        "analysis"
                    ):
