...
async def lifespan(app: FastAPI):
    ...
    if vision.get('available'):
        ...
        if analyzer_class:
            app.state.vision_analyzer = analyzer_class()
...
async def health_check():
    ...
    return {
        ...
        "components": {
            name: bool(comp) for name, comp in components.items()
        }
    }
...
def mount_routers():
    ...
    if monitoring.get('router'):
        app.include_router(monitoring['router'], prefix="/monitoring", tags=["monitoring"])
        logger.info("✅ Monitoring API mounted")

# Mount routers after components are loaded
