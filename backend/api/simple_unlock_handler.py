...
async def _analyze_command_context(command: str, action: str, jarvis_instance=None) -> Dict[str, Any]:
...
async def _generate_contextual_response(action: str, context: Dict[str, Any], response_gen) -> str:
    ...
    if context.get('urgency') == 'high':
        return "Right away! " + ("Unlocking your screen now." if action == "unlock_screen" else "Locking your screen now.")

    # Default fast responses
    return "Of course, Sir. " + ("Unlocking for you." if action == "unlock_screen" else "Locking for you.")
...
async def _execute_screen_action(action: str, context: Dict[str, Any], jarvis_instance=None) -> Dict[str, Any]:
...
async def _try_keychain_unlock(context: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if result.returncode == 0:
        ...
        return unlock_result, "Keychain unlock successful" if unlock_result else "Keychain unlock failed"
...
async def _generate_command_error_response(analysis: Dict[str, Any], response_gen) -> Dict[str, Any]:
    ...
    if response_gen:
        try:
            error_response = response_gen.get_error_message(
                "invalid_command", 
                "I didn't understand that screen command"
            )
        except:
...
async def _enhance_result_with_context(result: Dict[str, Any], response_text: str,
                                     action: str, context: Dict[str, Any], response_gen) -> Dict[str, Any]:
...
async def _generate_troubleshooting_info(action: str, result: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
