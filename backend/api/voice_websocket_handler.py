...
async def handle_websocket_message(
    message: Dict[str, Any], context: Dict[str, Any] = None
) -> Dict[str, Any]:
    """
...
async def handle_start_listening(
    ...
    try:
        ...
        return {"type": "listening_status", "listening": False, "error": str(e)}
...
async def handle_process_audio(
    message: Dict[str, Any], context: Dict[str, Any] = None
) -> Dict[str, Any]:
    """Process audio data"""
    voice_system = get_voice_system()
    ...
    try:
        ...
        return {"type": "audio_result", "success": False, "error": str(e)}
...
async def handle_voice_command(
    ...
    logger.info(
        f"[WEBSOCKET DEBUG] handle_voice_command called with message: {message}"
    )

    jarvis = get_jarvis_voice()

    if not jarvis:
        logger.error("[WEBSOCKET DEBUG] JARVIS voice instance is None!")
        return {
            "type": "command_result",
            "success": False,
    ...
    try:
        ...
        return {
            ...
            "response": response,
            "action_taken": (
                jarvis.last_action if hasattr(jarvis, "last_action") else None
            ),
        }

    except Exception as e:
        logger.error(f"Voice command error: {e}")
        return {"type": "command_result", "success": False, "error": str(e)}
...
async def handle_set_voice(
    ...
    try:
        ...
        return {"type": "voice_settings", "success": False, "error": str(e)}
...
async def handle_get_voices(
    ...
    try:
        voices = (
            voice_system.get_available_voices()
            if hasattr(voice_system, "get_available_voices")
            else []
        )
        ...
        return {"type": "available_voices", "voices": [], "error": str(e)}
...
async def handle_start_stream(
    ...
    return {"type": "stream_started", "stream_id": stream_id, "success": True}
...
async def handle_audio_chunk(
    ...
    if not stream_id or stream_id not in audio_streams:
        return {"type": "chunk_result", "success": False, "error": "Invalid stream ID"}
    ...
    try:
        ...
        if message.get("process_partial", False):
            ...
            if voice_system:
                ...
                partial_result = await voice_system.process_audio(
                    audio_data, partial=True
                )
        ...
        return {"type": "chunk_result", "success": False, "error": str(e)}
...
async def handle_end_stream(
    ...
    if not stream_id or stream_id not in audio_streams:
        return {"type": "stream_result", "success": False, "error": "Invalid stream ID"}
    ...
    try:
        ...
        if stream_id in audio_streams:
            del audio_streams[stream_id]
        return {"type": "stream_result", "success": False, "error": str(e)}
