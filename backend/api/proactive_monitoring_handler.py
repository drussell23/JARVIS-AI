...
class ProactiveMonitoringHandler:
    ...
    async def handle_monitoring_request(self, command: str) -> Dict[str, Any]:
        ...
        return {
            "handled": False,
            "reason": "Not a monitoring command"
        }
    ...
    async def enable_change_reporting(self) -> Dict[str, Any]:
        ...
        if not self.monitoring_active and self.vision_handler.intelligence:
            ...
            if hasattr(self.vision_handler.intelligence, 'start_multi_space_monitoring'):
                success = await self.vision_handler.intelligence.start_multi_space_monitoring()
                if success:
...
The purple indicator confirms I'm actively watching. What specific changes are most important to you?"""
        ...
        while self.monitoring_active and self.report_changes:
            try:
                ...
                if self.last_workspace_state:
                    # Detect changes
                    changes = self._detect_changes(self.last_workspace_state, current_state)
    ...
    async def _get_workspace_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.vision_handler.intelligence, 'multi_space_detector'):
                window_data = self.vision_handler.intelligence.multi_space_detector.get_all_windows_across_spaces()
    ...
    def _detect_changes(self, old_state: Dict[str, Any], new_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Space changes
        if old_state.get('current_space', {}).get('id') != new_state.get('current_space', {}).get('id'):
            changes.append({
                'type': 'space_changed',
    ...
    def _format_change_message(self, change: Dict[str, Any]) -> str:
        ...
        if change_type == 'window_opened':
            return f"I notice you've opened {change['app']} on Desktop {change['space']}"
