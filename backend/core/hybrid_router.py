...
class HybridRouter:
    ...
    def route(self, context: RoutingContext) -> Tuple[RouteDecision, Optional[str], Dict[str, Any]]:
        ...
        for rule in self.rules:
            if self._matches_rule(rule, context):
                ...
                logger.debug(f"Routed '{context.command}' via rule '{rule['name']}' → {decision.value}")
    ...
    def _matches_rule(self, rule: Dict, context: RoutingContext) -> bool:
        ...
        if 'capabilities' in match_config:
            required_caps = match_config['capabilities']
    ...
    def _extract_keywords(self, command: str) -> List[str]:
        ...
        stop_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can'}
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_confidence': sum(h['metadata'].get('confidence', 0) for h in self.routing_history) / total if total > 0 else 0
        }
