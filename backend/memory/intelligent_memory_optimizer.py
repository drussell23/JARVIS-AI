...
class ProcessInfo:
    ...
    def _determine_killable(self) -> bool:
        ...
        killable_patterns = [
            ...
            "com.apple.WebKit", "VTDecoderXPCService",
    ...
    def _determine_suspendable(self) -> bool:
        ...
        suspendable_patterns = [
            ...
            "Reminders", "Notes", "Calendar", "WhatsApp",
            "Telegram", "Signal", "Messages", "FaceTime"
        ]
...
class IntelligentMemoryOptimizer:
    ...
    async def optimize_for_langchain(self, aggressive: bool = False) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"Starting {'aggressive' if aggressive else 'intelligent'} memory optimization for LangChain")
    ...
    async def _optimize_python_memory(self) -> float:
        ...
        functools._lru_cache_wrapper.cache_clear = lambda self: self.cache_clear()
    ...
    async def _close_high_memory_applications(self) -> float:
        ...
        for app in high_memory_apps:
            ...
            if "cursor" in app.name.lower() or "code" in app.name.lower():
                ...
                logger.info(f"Attempting to close {app.name} (using {app.memory_percent:.1f}% memory)")
                ...
                if self.is_macos:
                    try:
                        ...
                        logger.info(f"Gracefully closed {app.name}, freed {app.memory_mb:.0f} MB")
            ...
            if current_mem <= self.target_memory_percent:
                logger.info(f"Target memory reached after closing {len(closed_apps)} apps")
                break
    ...
    async def get_optimization_suggestions(self) -> List[str]:
        ...
        if self.is_macos:
            try:
                ...
                if tab_count > 5:
                    suggestions.append(f"Close some Chrome tabs (currently {tab_count} open)")
            except:
                pass
        ...
        for proc in psutil.process_iter(['name', 'memory_percent']):
            try:
                if proc.info['memory_percent'] > 5:
                    name = proc.info['name']
                    if any(ide in name.lower() for ide in ['cursor', 'code', 'intellij']):
                        suggestions.insert(0, f"Close {name} IDE (using {proc.info['memory_percent']:.1f}% memory)")
                    elif any(app in name.lower() for app in ['whatsapp', 'slack', 'discord']):
                        suggestions.append(f"Close {name} (using {proc.info['memory_percent']:.1f}% memory)")
            except:
                continue
    ...
    async def optimize_for_mode(self, target_mode: str = "langchain") -> Dict:
        ...
        if target_mode == "langchain":
            ...
            return {
                "success": success,
                "report": report,
                "message": "Memory optimization completed" if success 
